...
/**
 * A hook used to format RRFlowQL structures into strings
 */
export function useResultFormatter(): [Formatter, ResultFormat] {
	...
	const formatter = useStable(async (value: any) => {
		return await getRRFlowQL().formatValue(value, format === "json", true);
	});
...
/**
 * Returns whether the given value is valid RRFlowQL or not
 *
...
export function useValueValidator(value: string, objectRoot?: boolean): [boolean, any, boolean] {
	...
	useEffect(() => {
		...
		const validate = async () => {
			setIsLoading(true);
			try {
				const value = await getRRFlowQL().parseValue(bodyCache);
